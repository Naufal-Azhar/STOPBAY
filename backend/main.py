...
Ticketless Smart Parking System — HTTP-only + Camera Stream Proxy
...
Endpoints:
  ...
  GET    /api/parking/by-plate/{plate}     (v3)
...
def register_card(req: RegisterCardRequest, db: Session = Depends(get_db)):
    ...
    if req.plate_number:
        session = session.filter(ActiveParking.plate_number == req.plate_number)
    session = session.order_by(ActiveParking.entry_time.desc()).first()
